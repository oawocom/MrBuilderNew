...
export const specPayload = (p: PergolaSpec, encCodes: Record<string, string> = {}, accCodes: Record<string, string> = {}) => ({
  ...
  pergola_spec: { structure_type: structureCode(p.type), type: p.type, brand: p.brand, enclosures: p.encOn ? p.enc.map((e) => ({ type: encCodes[e.type] ?? enclosureCode(e.type), label: e.type, width_ft: toFt(e.w, p.units), length_ft: toFt(e.l, p.units), height_ft: toFt(e.h, p.units), location: e.loc })) : [],
    accessories: Object.entries(p.acc).filter(([, n]) => n > 0).map(([k, qty]) => ({ type: accCodes[k] ?? accessoryCode(k), label: k, qty })), footings: { involved: p.footOn, ready: p.footReady, count: Number(p.footN || 0) }, photos: p.photos, plans: p.plans },
});
